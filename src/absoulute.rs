...
impl AbsoluteKeys for KeyVec {
    ...
    //FAKES
        fn get_fake(&self, key:usize) -> Result<Option<usize>,Errors> {
            Ok(self.fakes.get(key).ok_or(Errors::FakeKeyOutOfBounds)?.clone())
        }
    ...
    //REALS FAKE
        ...
        fn get_reals_fake(&self,key:usize) -> Result<usize,Errors> {
            Ok(*self.reals_fake.get(key).ok_or(Errors::RealsFakeKeyOutOfBounds)?)
        }

        fn mod_to_reals_fake(& mut self,key:usize) -> Result<&mut usize,Errors> {
            Ok(self.reals_fake.get_mut(key).ok_or(Errors::RealsFakeKeyOutOfBounds)?)
        }
